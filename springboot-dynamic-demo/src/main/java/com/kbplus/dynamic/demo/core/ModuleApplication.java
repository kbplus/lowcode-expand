...
import com.kbplus.dynamic.demo.utils.ReflectUtil;
...
public class ModuleApplication {
    ...
    Map<String, Object> extObjMap =new HashMap<>();
    ...
    public void reloadJar(URL url, ApplicationContext applicationContext,SqlSessionFactory sqlSessionFactory) throws Exception {
         ...
         */
        classMap = moduleClassLoader.load();

        MapperLoader mapperLoader = new MapperLoader();

        xmlBytesMap=moduleClassLoader.getXmlBytesMap();
        /**
         * 刷新mybatis的xml和Mapper接口资源，Mapper接口其实就是xml的namespace
         */
        extObjMap = mapperLoader.refresh(sqlSessionFactory, xmlBytesMap);
        /**
         * 将各种资源放入spring容器
         */
...
//                .apis(RequestHandlerSelectors.basePackage(basePackage)).paths(PathSelectors.any()).build();
    ...
    private ApiInfo apiInfo() {
        return new ApiInfoBuilder()
                ...
                //创建人
                .contact(new Contact("kbplus", "https://www.cnblogs.com/xx", "xx@qq.com"))
                //版本号
                .version("1.0")
                //描述
     ...
     * 删除bean注册
     */
    public void removeBeans(ApplicationContext applicationContext,SqlSessionFactory sqlSessionFactory) throws Exception {

        MapperLoader mapperLoader = new MapperLoader();

        if(xmlBytesMap!=null&&xmlBytesMap.size()>0) {
            /**
             * 刷新mybatis的xml和Mapper接口资源，Mapper接口其实就是xml的namespace
             */
            extObjMap = mapperLoader.removeRegisterMapper(sqlSessionFactory, xmlBytesMap);
        }
        /**
         * 将各种资源放入spring容器
         */
        removeRegisterBeans(applicationContext, classMap, extObjMap);
     ...
     * @param cacheClassMap
     */
    public void removeRegisterBeans(ApplicationContext applicationContext, Map<String, Class<?>> cacheClassMap,Map<String,Object> extObjMap) throws Exception {
        /**
         * 将applicationContext转换为ConfigurableApplicationContext
         */
        ConfigurableApplicationContext configurableApplicationContext = (ConfigurableApplicationContext) applicationContext;
